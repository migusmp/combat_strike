...
interface CourseReviewsProps {
  userReviews: UserReview[] | null; // puede ser null también
...
export default function CourseReviews({
  ...
  const handleShowMore = () => {
    setVisibleCount((prev) => Math.min(prev + max, safeReviews.length));
  };
  ...
  return (
    <section className={style.courseReviews}>
      ...
      {safeReviews.length === 0 ? (
        // 🟡 Si no hay reseñas, mostramos el mensaje
        <p
          style={{
            color: "#aaa",
            fontStyle: "italic",
        ...
        >
          Este curso no tiene valoraciones
        </p>
      ) : (
        <>
          <div className={style.reviewsGrid}>
            {safeReviews.slice(0, visibleCount).map((review, idx) => (
          ...
          {visibleCount < safeReviews.length ? (
            <button onClick={handleShowMore} className={style.showMoreBtn}>
              Mostrar todas las reseñas
            </button>
          ) : (
            safeReviews.length > max && (
              <button onClick={handleShowLess} className={style.showMoreBtn}>
                Ver menos
              </button>
            )
          )}
